...
def conv2d_same(inputs, num_outputs, kernel_size, stride, scope=None):
    if stride == 1:
        return slim.conv2d(inputs, num_outputs, [kernel_size, kernel_size], stride=1,
        ...
        return slim.conv2d(inputs, num_outputs, [kernel_size, kernel_size], stride=stride,
...
def resnet_arg_scope(is_training=True, weight_decay=1e-4, batch_norm_decay=0.997,
                     batch_norm_epsilon=1e-5, batch_norm_scale=True):
    batch_norm_params = {'is_training':is_training, 'decay':batch_norm_decay,
...
def bottleneck(inputs, depth, depth_bottleneck, stride, outputs_collections=None,
    ...
    with tf.variable_scope(scope,'bottleneck_v2', [inputs]) as sc:
        depth_in = slim.utils.last_dimension(inputs.get_shape(), 4)
        preact = slim.batch_norm(inputs, activation_fn=tf.nn.relu, scope='preact')
        if depth == depth_in:
            shortcut = subsample(inputs, stride, 'shortcut')
        else:
            shortcut = slim.conv2d(preact, depth, [1, 1], stride=stride, activation_fn=None,
                                   normalizer_fn=None, scope='shortcut')
        residual = slim.conv2d(preact, depth_bottleneck, [1, 1], stride=1, scope='conv1')
        residual = conv2d_same(residual, depth_bottleneck, 3, stride, 'conv2')
        residual = slim.conv2d(residual, depth, [1, 1], stride=1, activation_fn=None,
                               normalizer_fn=None, scope='conv3')
...
def resnet_v2_50(inputs, num_classes=None, global_pool=True, reuse=None, scope='resnet_v2_50'):
    ...
    return resnet_v2(inputs, blocks, num_classes, global_pool, True, reuse, scope)
